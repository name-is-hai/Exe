...
import {
  ...
  getDocsChats,
  setQueryChats,
  subscribeToQueryChats,
  updateDocsUserChats,
  setDocsChats,
  setQueryUserChats,
  subscribeToQueryUserChats,
...
export function ChatLayout({
  ...
  useEffect(() => {
    if (queryParam.get('room')) {
      const select = JSON.parse(decodeURIComponent(atob(queryParam.get('room'))));
      select.avatar =
        select.avatar ??
        'https://media.istockphoto.com/id/961516280/vector/house-flat-icon.jpg?s=612x612&w=0&k=20&c=KsZmFORfrbq5i3cSrCfm3do88Q-0MWw5kjYEg1JSszg=';
  ...
  useEffect(() => {
    if (selectedUser) {
      const combinedId = user.uid > selectedUser.uid ? user.uid + selectedUser.uid : selectedUser.uid + user.uid;
      getDocsChats([combinedId])
        .then((res) => {
          if (!res.exists()) {
            const currentUser: UserMessage = {
              uid: user.uid,
              name: user.display_name ?? user.phone,
              avatar: user.photo ?? 'https://shadcn-chat.vercel.app/User1.png',
            };
  ...
  return (
    ...
    >
      <ResizablePanel
        ...
        className={cn(isCollapsed && 'min-w-[60px] md:min-w-[70px] transition-all duration-300 ease-in-out')}
      >
