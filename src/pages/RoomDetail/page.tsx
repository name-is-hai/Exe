...
export function RoomDetail() {
    ...
    const { data: rooms, error: errorRooms, isLoading: isLoadingRooms } = usePost(['roomlist', query.get('id')], '/exe/rooms/get-list')
    const { data: room, error: errorRoom, isLoading: isLoadingRoom } = usePost(['roomdetail', query.get('id')], '/exe/rooms/detail', { id: parseInt(query.get('id') || '') })
    ...
    return (
        <Page>
            <Container>
                        ...
                        >
                            <CarouselContent>
                                {Array.from({ length: 5 }).map((_, index) => (
                                    <CarouselItem key={index} className="w-9 basis-1/2 md:basis-1/3 md:w-14">
                                        <Card>
                        ...
                        <div>
                            <Show>
                                ...
                                <Show.Else>
                                    ...
                                    <h2 className="flex items-center mt-2 text-sm font-medium leading-none"><MapPinned className="w-5 h-5" />{room?.data.address}</h2>
                                    <Separator className="my-4" />
                                    <div className="flex items-center h-5 text-sm space-x-7">
                                        <div className="text-sm font-medium leading-none">Chỉ Từ: {numberCurrencyFormat(room?.data.price)} / Tháng</div>
                ...
                <TopRoom rooms={clearRooms(rooms)} isLoading={isLoadingRooms} />
            </Container>
        </Page >
    )
}